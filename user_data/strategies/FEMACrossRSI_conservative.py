"""FEMACrossRSI conservative variant: ema_fast=24, ema_slow=60, ema_trend=150, rsi_period=18, rsi_min=42, rsi_max=72, vol_sma_period=25"""
import pandas as pd
...
class FEMACrossRSI_conservative(IStrategy):
    ...
    ema_fast       = IntParameter(10, 30,  default=24,  space="buy", optimize=False, load=False)
    ema_slow       = IntParameter(30, 80,  default=60,  space="buy", optimize=False, load=False)
    ema_trend      = IntParameter(80, 200, default=150, space="buy", optimize=False, load=False)
    rsi_period     = IntParameter(7, 21,   default=18,  space="buy", optimize=False, load=False)
    rsi_min        = DecimalParameter(30, 50, decimals=0, default=42, space="buy", optimize=False, load=False)
    rsi_max        = DecimalParameter(60, 80, decimals=0, default=72, space="buy", optimize=False, load=False)
    vol_sma_period = IntParameter(10, 30,  default=25,  space="buy", optimize=False, load=False)
